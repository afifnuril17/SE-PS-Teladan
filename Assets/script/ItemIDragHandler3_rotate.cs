...
public class ItemIDragHandler3_rotate : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler, IBeginDragHandler, IEndDragHandler
{
	...
	{
		if (o_panel.GetComponent<scrollrectpos2>().bawah == false || bentuk_image.transform.parent == GameObject.Find("objek_dibuat").transform)
		{
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
		...
		waktu = 0;
		o_panel.GetComponent<scrollrectpos2>().balik();
		...
		if(o_panel.GetComponent<scrollrectpos2>().bawah == true)
        {
        ...
        else
        {
			...
			o_panel.GetComponent<scrollrectpos2>().kebawah();
		}
		//Debug.Log(play);
		if (bentuk_image.transform.position != startPosition)
		{
