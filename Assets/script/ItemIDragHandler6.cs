...
public class ItemIDragHandler6 : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler, IBeginDragHandler, IEndDragHandler
{
	...
	void Update()
	{
		...
		//Debug.Log(scroll.transform.position.x);
		//Debug.Log(bentuk_image.transform.position);
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
		...
		if (transform.position != startPosition)
		{
            ...
            {
				//bentuk_image.SetActive(true);
				slot.transform.position = awal;
				itemBeingDragged = null;
				float selisih = awal.x - scroll.transform.position.x;
				Debug.Log(selisih);
				awal.x = awal.x - selisih+231;
				Debug.Log(awal.x - selisih);
				Debug.Log("awal.x"+ awal.x);
    ...
    {
		if(bentuk_image.transform.parent == GameObject.Find("play_area").transform)
        {
			Debug.Log("masuk play area");

			if (coll.gameObject.CompareTag("play_area"))
				{
					triger = "tidak"; bentuk_image.
					transform.parent = objek_area.transform;
					bentuk_image.transform.position = awal; ;
				}
        }
		
		/*
		if (awalparent == "objek_area (UnityEngine.RectTransform)")
		{
			

			if (GameObject.Find("btn_manager").GetComponent<btn_manager>().hal1 == "aktif")
            {
				bentuk_image.SetActive(true);
				bentuk_image.transform.parent = GameObject.Find("objek_area").transform;
				Debug.Log(bentuk_image.transform.parent);
		...
		if (awalparent == "objek_area2 (UnityEngine.RectTransform)")
		{

			if (GameObject.Find("btn_manager").GetComponent<btn_manager>().hal2 == "aktif")
			{
				bentuk_image.SetActive(true);
				bentuk_image.transform.parent = GameObject.Find("objek_area2").transform;
				Debug.Log(bentuk_image.transform.parent);
			}
            else
			{
		...
		if (awalparent == "objek_area3 (UnityEngine.RectTransform)")
		{

			if (GameObject.Find("btn_manager").GetComponent<btn_manager>().hal3 == "aktif")
			{
				bentuk_image.SetActive(true);
				bentuk_image.transform.parent = GameObject.Find("objek_area3").transform;
				Debug.Log(bentuk_image.transform.parent);
			}
            else
			{
