...
public class scrollrectpos4A : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (bawah == true)
        {
            ...
            for (int i = 0; i <= bentuk_image.Length - 1; i++)
            {
                if (bentuk_image[i].transform.parent == scroll.transform)
                {
                    ...
                    bentuk_image[i].GetComponent<ItemIDragHandler4A_rotate>().btn_rotate.SetActive(false);
                    bentuk_image[i].GetComponent<ItemIDragHandler4A_rotate>().btn_flip.SetActive(false);
                }
            }
        }
        else
        {
            ...
            for (int i = 0; i <= bentuk_image.Length - 1; i++)
            {
                if (bentuk_image[i].transform.parent == scroll.transform)
                {
                    bentuk_image[i].GetComponent<ItemIDragHandler4A_rotate>().enabled = true;
                }
            }
        }
    ...
    public void keatas()
    {
        panelscrol.transform.position = new Vector3(transform.position.x, transform.position.y * 6f, transform.position.z);
