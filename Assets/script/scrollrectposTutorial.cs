...
public class scrollrectposTutorial : MonoBehaviour
{
    ...
    public GameObject collider_bawah;
    public int j;
    public int ups = 0;
    private bool up;
        ...
        if (bawah == true)
        {
            ...
            for (int i = 0; i <= bentuk_image.Length - 1; i++)
            {
                if (bentuk_image[i].transform.parent == scroll.transform)
                {
                    ...
                    bentuk_image[i].GetComponent<ItemIDragHandler_Tutorial>().btn_rotate.SetActive(false);
                    bentuk_image[i].GetComponent<ItemIDragHandler_Tutorial>().btn_flip.SetActive(false);
        ...
        else
        {
            ...
            for (int i = 0; i <= bentuk_image.Length -1; i++)
            {
                if (bentuk_image[i].transform.parent == scroll.transform)
                {
                    ...
                    bentuk_image[i].GetComponent<ItemIDragHandler_Tutorial>().enabled = true;
                }
            }
        }
    ...
    public void keatas()
    {
        ...
        collider_bawah.SetActive(false);
        btn_up.SetActive(false);
