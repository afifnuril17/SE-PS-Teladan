...
public class ItemIDragHandler5A_rotate : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler, IBeginDragHandler, IEndDragHandler
{
	...
	{
		if (o_panel.GetComponent<scrollrectpos5A>().bawah == false || bentuk_image.transform.parent == GameObject.Find("objek_dibuat").transform)
		{
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
		...
		waktu = 0;
		o_panel.GetComponent<scrollrectpos5A>().balik();
		...
		if(o_panel.GetComponent<scrollrectpos5A>().bawah == true)
        {
        ...
        else
        {
			...
			o_panel.GetComponent<scrollrectpos5A>().kebawah();
		}
		//Debug.Log(play);
		if (bentuk_image.transform.position != startPosition)
		{
