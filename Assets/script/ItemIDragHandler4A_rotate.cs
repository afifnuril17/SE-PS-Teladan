...
public class ItemIDragHandler4A_rotate : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler, IBeginDragHandler, IEndDragHandler
{
	...
	{
		if (o_panel.GetComponent<scrollrectpos4A>().bawah == false || bentuk_image.transform.parent == GameObject.Find("objek_dibuat").transform)
		{
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
		...
		waktu = 0;
		o_panel.GetComponent<scrollrectpos4A>().balik();
		...
		if(o_panel.GetComponent<scrollrectpos4A>().bawah == true)
        {
        ...
        else
        {
			...
			o_panel.GetComponent<scrollrectpos4A>().kebawah();
		}
	...
	void OnTriggerExit2D(Collider2D coll)
	{
		if (bentuk_image.transform.parent == GameObject.Find("objek_dibuat").transform)
		{
			...
			if (coll.gameObject.CompareTag("play_area"))
			{
				...
				Debug.Log("Balik ke awal play area");
				//EdgesScroll.enabled = true;
				bentuk_image.transform.SetParent(o_awal_panel.transform);
				bentuk_image.transform.position = awal;
				//bentuk_image.GetComponent<Rigidbody2D>().bodyType = RigidbodyType2D.Kinematic;
