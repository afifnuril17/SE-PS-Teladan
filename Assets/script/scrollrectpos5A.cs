...
public class scrollrectpos5A : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else
        {
            ...
            for (int i = 0; i <= bentuk_image.Length -1; i++)
            {
                if (bentuk_image[i].transform.parent == scroll.transform)
                {
                    bentuk_image[i].GetComponent<ItemIDragHandler5A_rotate>().enabled = true;
                }
            }
        }
    ...
    public void keatas()
    {
        panelscrol.transform.position = new Vector3(transform.position.x, transform.position.y * 8f, transform.position.z);
