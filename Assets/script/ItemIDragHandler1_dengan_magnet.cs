...
public class ItemIDragHandler1_dengan_magnet : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler, IBeginDragHandler, IEndDragHandler
{
	...
	{
		//if (o_panel.GetComponent<scrollrectpos>().bawah == false || bentuk_image.transform.parent == GameObject.Find("objek_dibuat").transform)
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
		...
		if (o_panel.GetComponent<scrollrectpos>().bawah == true)
		{
		...
		else
		{
			...
			o_panel.GetComponent<scrollrectpos>().kebawah();
		}
